...
// 1 :
// print contohObj dengan key name ambil dari index 2 contohArray
console.log(`Key name: ${contohObj.name}`);
// print contohObj dengan key age ambil dari index 3 contohArray
console.log(`Key age: ${contohObj.age}`);
// print contohObj dengan key id ambil dari index 1 contohArray
console.log(`Key id: ${contohObj.id}`);

// 2 :
// - kalian tambah data kalian di arr + 1 temen kalian juga
contohArray.push(3, "Ranto", 25);
...
for (let i = 0; i < contohArray.length; i++) {
  if (i == 0 || i == 3 || i == 6) {
    ...
    console.log(`index ${[i]} berisikan ${contohObj.id}`);
  }
}
...
// dari array kosong yang udah di masukkan data diatas, tapi cuman print yang age nya dibawah 28 dan id nya  (loping dan if) 2
let data = [];
...
const arr = [obj1, obj2, obj4, obj5, obj6, obj7];
...
for (let i = 0; i < data.length; i++) {
  console.log(`Name: ${data[i].name}, Age: ${data[i].age}, ID: ${data[i].id}`);
}
